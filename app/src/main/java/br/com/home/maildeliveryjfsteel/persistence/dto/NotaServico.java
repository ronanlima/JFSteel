...
public class NotaServico extends GenericDelivery {
    ...
    public ContentValues getValuesInsert(Context context) {
        ...
        if (getLeitura() != null) {
            values.put(context.getString(R.string.leitura), getLeitura());
        }
        if (getMedidorExterno() != null) {
            values.put(context.getString(R.string.medidor_externo), getMedidorExterno().equalsIgnoreCase("sim") ? 1 : 0);

        }
        if (getMedidorVizinho() != null) {
            values.put(context.getString(R.string.medidor_vizinho), getMedidorVizinho());
